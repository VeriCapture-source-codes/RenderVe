...
export const createReply = async (req, res, next) => {
  ...
  if (!userId || !commentId) {
    return next(
      new ApiError(
        403,
        "You are not authorized. Please Login and click on the comment to reply"
      )
    );
  }
...
export const updateReply = async (req, res, next) => {
  ...
  if (!userId || !replyId) {
    return next(
      new ApiError(
        403,
        "You are not authorized. Please Login and click on the reply you want to update"
      )
    );
  }
  ...
  const replyToUpdate = await replyModel.findByIdAndUpdate(
    reply._id,
    {
      text,
    },
...
export const deleteReply = async (req, res, next) => {
  ...
  const replyToDelete = await replyModel.findOne({
    _id: replyId,
    user: loggedInUser._id,
  });
  if (!replyToDelete) {
...
export const replyLikes = async (req, res, next) => {
  const userId = req.user._id;
  const { replyId } = req.params;
  if (!userId || !replyId) {
    return next(
      new ApiError(
        403,
        "You are not authorized. Please Login and go the reply you want to like"
      )
    );
  }
